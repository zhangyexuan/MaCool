...
<html ng-app="app">
	<head>
		...
		<link rel="stylesheet" href="{{ URL::asset('Home/css/reset.css') }}">
		<style type="text/css">
			html {
				width: 100%;
				height: 100%;
	...
	<body>
		<div id="root" class="container">
			...
			<div class="m-button-bar">
				<ul class="u-tab">
					<li id="prev_btn"><a href="{{$Page_S}}">上一章</a></li>
					<li id="next_btn"><a href="{{$Page_N}}">下一章</a></li>
				</ul>
			</div>
			...
			<div class="bottom-nav" id="bottom_nav" style="display: none;">
				<div class="bottom-nav-item" id="catalog_btn">
					<div class="item-wrap">
				...
				<div class="bottom-nav-item" id="font_btn">
					<div class="item-wrap">
				...
				<div class="bottom-nav-item" id="night_day_switch_btn">
			...
			<!-- 底部面板 -->
			<div class="nav-pannel-bk nav-pannel" style="display: none;"></div>
			<div id="font-container" class="nav-pannel" style="display: none;">
				...
				<div class="child-mod">
					<span>背景</span>
					<div class="bk-container bg-1">
						<div class="bk-container-active"></div>
					</div>
					<div class="bk-container bg-2">
					</div>
					<div class="bk-container bg-3">
					</div>
					<div class="bk-container bg-4">
					</div>
				</div>
			</div>
		</div>
		<script src="{{ URL::asset('Home/lib/zepto.min.js') }}"></script>
		<script>
			window.jQuery = $;
		</script>
		<script src="{{ URL::asset('Home/js/jquery.base64.js') }}"></script>
		<script src="{{ URL::asset('Home/js/jquery.jsonp.js') }}"></script>
...
// >>>>>>> c87d1f73123e7a0f39caa493f1b302f1fb04337a
					var getFictionInfo = function(callback) {
						$.get('{{$AjaxData}}', function(data) {
							$('#fiction_container').html(data.content);
							$('#fiction_chapter_title').html(data.name);
							// 获得章节信息之后的回调
					...
					return {
						init: init,
						prevChapter: prevChapter,   // 上翻页接口
						nextChapter: nextChapter    // 下翻页接口
					}
				}
				...
				function ReaderBaseFrame(container) {
					...
					return function(data) {
						...
						container.css('background_color', initBgColor);
					}
				}
				...
				function EventHandler() {		
					...
					$('#action_mid').click(function(){
						if(Dom.top_nav.css('display') == 'none') {
							...
							Dom.top_nav.hide();
							Dom.nav_pannel.hide();
							Dom.font_icon.removeClass('icon-active');
						}
					});
					...
					Dom.font_btn.click(function() {
						if(Dom.nav_pannel.css('display') == 'none') {
							Dom.nav_pannel.show();
							Dom.font_icon.addClass('icon-active');
						}else{
							Dom.nav_pannel.hide();
							Dom.font_icon.removeClass('icon-active');
						}
					...
					// 白天夜间转换即触发背景切换事件
					$('#night_day_switch_btn').click(function() {
						if($('#day_icon').css('display') == 'none') {
							$('#day_icon').show();
							$('#night_icon').hide();
							RootContainer.css('background-color', 'rgba(0, 0, 0, 0.8)');
							Dom.bk_container.html('');
							$('.bk-container:last-child').html('<div class="bk-container-active"></div>');
							...
							Dom.bk_container.html('');
							$('.bk-container:first-of-type').html('<div class="bk-container-active"></div>');
						}
					});
					...
					Dom.bk_container.click(function() {
						Dom.bk_container.html('');
						$(this).html('<div class="bk-container-active"></div>');
						//alert($(this).css('background-color'))
						initBgColor = $(this).css('background-color');
						RootContainer.css('background-color', initBgColor);
